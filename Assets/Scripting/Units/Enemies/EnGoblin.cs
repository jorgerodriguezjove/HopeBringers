...
public class EnGoblin : EnemyUnit
{
    ...
    IEnumerator MovingUnitAnimation()
    {
        ...
        for (int j = 1; j < pathToObjective.Count-1; j++)
        {
            ...
            currentTileVectorToMove = new Vector3(pathToObjective[j].transform.position.x, pathToObjective[j].transform.position.y + 1, pathToObjective[j].transform.position.z);
